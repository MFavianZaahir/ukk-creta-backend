...
class TokenAuthentication(BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        if auth[0].lower() != prefix.lower():
            logger.warning(f"Invalid token prefix. Expected: {prefix}, Got: {auth[0]}")
            return None
        if len(auth) == 1:
            ...
            logger.error("Authentication failed: Token contains spaces.")
            raise exceptions.AuthenticationFailed(msg)
    ...
    def authenticate_credentials(self, token):
        ...
        for auth_token in get_token_model().objects.filter(
            ...
            try:
                ...
                logger.debug(f"Computed digest: {digest}")
            ...
            if compare_digest(digest, auth_token.digest):
                ...
                logger.info(f"User {auth_token.user.username} authenticated with token.")
                return self.validate_user(auth_token)

        logger.warning("Token authentication failed: Invalid token.")
        raise exceptions.AuthenticationFailed(msg)
